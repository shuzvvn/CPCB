...
for epoch in range(1):
	...
	beta_start = np.delete(beta, 0, 1)
	loss = -sum(y*np.log(y_hat)) + (lamda*((alpha_start**2).sum())) + (lamda*((beta_start**2).sum()))
	print(epoch, loss, y_hat)
	...
	# L2
	beta = beta - (eta * (d_l_beta + (2*lamda*beta)))
	...
	# L2
	alpha = alpha - (eta * (d_l_alpha + (2*lamda*alpha)))