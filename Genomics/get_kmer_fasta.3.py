...
def kmer_position(kmer_h, seq_str):
	forward_pos = [m.start(0) for m in re.finditer(kmer_h, seq1)]
	...
	reverse_pos = [m.start(0) for m in re.finditer(rc_kmer_h, seq1)]
...
def main():
	...
	kmer_tab = kmer_tab[kmer_tab['observed']>10]
	kmer_tab = kmer_tab[['corrected', 'observed', 'expected']]
	kmer_tab['x'] = kmer_tab.index
	ax1 = kmer_tab.plot(kind='scatter', x='x', y='observed', color='b', label='observed')    
	ax2 = kmer_tab.plot(kind='scatter', x='x', y='expected', color='r', ax=ax1, label='expected')    
	ax3 = kmer_tab.plot(kind='scatter', x='x', y='corrected', color='g', ax=ax1, label='corrected')
	plt.xlabel('rank')
	plt.ylabel('count')
